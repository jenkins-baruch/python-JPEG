...
def compress_image(path, entropy=False):    # pragma: no cover
    ...
    print("paddings")
    y_padding = [matrix if matrix.shape == (8,8) else padding_matrix_to_8_8(matrix) for matrix in y_split]
    cb_padding = [matrix if matrix.shape == (8,8) else padding_matrix_to_8_8(matrix) for matrix in cb_split]
    cr_padding = [matrix if matrix.shape == (8,8) else padding_matrix_to_8_8(matrix) for matrix in cr_split]
...
def main(*argv):    # pragma: no cover
    ...
    parser.add_argument('-d', action='store_true', help='Request input for attach debugger')
    args = parser.parse_args()
    if args.d: input("Attach debugger and Enter: ")
